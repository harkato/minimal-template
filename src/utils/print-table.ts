                ...
                (row, index) => `
                  <tr style="background-color: ${index % 2 === 0 ? '#ffffff' : '#f5f5f5'};">
                    ...
                    <td class="torque-angle-col status-icon">
                      <img src="../assets/icons/printtable/${
                        row.torqueStatus === 0 ? 'check_circle.svg' : row.torqueStatus === 2 ? 'arrow_downward.svg' : 'arrow_upward.svg'
                      }
                      " class="icon-svg ${
                      row.torqueStatus === 0 ? 'status-ok' : row.torqueStatus === 2 ? 'status-warning' : 'status-error'
                      }" />
                      ${row.torque}                      
                    </td>
                    <td class="torque-angle-col status-icon">                      
                      <img src="../assets/icons/printtable/${
                        row.angleStatus === 0 ? 'check_circle.svg' : row.torqueStatus === 2 ? 'arrow_downward.svg' : 'arrow_upward.svg'
                      }
                      " class="icon-svg ${
                        row.angleStatus === 0 ? 'status-ok' : row.torqueStatus === 2 ? 'status-warning' : 'status-error'
                      }" />
                      ${row.angle}
                    </td>
                  </tr>
