...
// Novo fetchData com tratamento de erros
const fetchData = async (endpoint: string) => {
  try {
    const response = await axios.get(`${API_URL}/${endpoint}`);
    return response.data;
  } catch (error: any) {  // Tipagem do erro para melhor tratamento
...
// Nova fetchProgramsData com tratamento de erros
export const fetchProgramsData = async (endpoint: string, toolList: any[]) => {
  try {
    const response = await axios.get(`${API_URL}/${endpoint}`, {
      params: { toolList },
    ...
    return response.data;
  } catch (error: any) {  // Tipagem do erro
    // Tratamento de erro mais específico baseado no tipo de erro
    if (axios.isAxiosError(error)) {
      // Erros específicos do Axios (ex: erro de rede, erro do servidor)
      console.error("Erro Axios:", error.message);
      if (error.response) {
        // console.error("Dados da Resposta:", error.response.data);
        console.error("Status da Resposta:", error.response.status);
        if (error.response.status === 404) { // Exemplo de um erro customizado baseado no código de status
